...
export const AddEditPLaylist = ({ navigation, route }) => {
    ...
    useEffect(() => {
        setAvailableHabits(habits.filter(habit => !playlistHabits.includes(habit)));
    }, [playlistHabits]);

    return (
        <View>
            <Text>
                <Text style={styles.title}>{isEditMode ? "Edit" : "Add new"} playlist</Text>
            </Text>
            <Divider />
            {/* Name input */}
            ...
            <View style={styles.formLine}>
                <Text style={styles.formLineText}>
                    <MaterialIcons name="playlist-add-check" size={24} color={secondaryColor} />
                    Selected habits
                </Text>
                <View style={{ flexDirection: "row", flexWrap: "wrap" }}>
                    {playlistHabits.map((habit, index) => (
                        <View
                            key={index}
                            style={{
                        ...
                        >
                            <Text style={{ color: "white" }}>{habit}</Text>
                            ...
                            <MaterialIcons
                                ...
                                onPress={() => {
                                    setPlaylistHabits(
                                        playlistHabits.filter((item) => item !== habit)
                                    );
                                }}
                            />
                        </View>
